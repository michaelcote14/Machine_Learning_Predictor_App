def single_encoder_printer():
        ...
        for column in original_dataframe:
            print('column:',column)
            category_count = original_dataframe[column].value_counts()


            if original_dataframe[column].dtypes == 'object' and len(category_count) < 3:
                # ToDo make this encoder only take single category columns
                try:
        ...
        if original_dataframe[column].dtypes == 'object' and len(category_count) < 3:
            ...
            try:
                ...
                single_dataframe_after_drop.rename(
                    columns={single_dataframe_after_drop.columns[0]: one_hot_default_name[15: 30]}, inplace=True)
...
if __name__ == '__main__':
    single_encoder_printer()

# ToDo some of the columns aren't encoding. Fix this.
